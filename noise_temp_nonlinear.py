...
realtemp = np.array(real_temperatures, dtype = float)
sys_noise_t = np.zeros((len(realtemp), len(x_all_temp)))
sig_sys_noise_t = np.zeros((len(realtemp), len(x_all_temp)))

def fit_func(x,a,b):
#    return b*(a*(T_term.n - x)+ x + T_amp.n)
#    return b*(a*np.exp(-np.sqrt(x/100))*(T_term.nominal_value - x) + x + T_amp.nominal_value)
    return b*(np.exp(-a*np.sqrt(x))*(T_term.n - x) + x + T_amp.n)
#    return b*(a*np.exp(-x)*(T_term.nominal_value - x) + x + T_amp.nominal_value)
...
#    sig_z = np.sqrt(np.diagonal(cov))
    print(realtemp)
    print(power_t)
    plt.plot(realtemp, power_t, 'o')
    popt, pcov = curve_fit(fit_func, realtemp, power_t, p0 = [.05, .01 ], bounds = (0,np.inf))
    print(popt)
    plt.plot(realtemp, fit_func(realtemp,popt[0], popt[1]))
#    plt.show()
    continue
